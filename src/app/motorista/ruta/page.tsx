...
import { distanceKm, orderPoint, prioritizeDeliveryOrders, type RoutePoint } from '@/lib/logistics/route-priority';
...
export default function RutaPage() {
  ...
  const [showPriorityMap, setShowPriorityMap] = useState(false);
  const [mapLocation, setMapLocation] = useState<RoutePoint | null>(null);
  ...
  const handleTogglePriorityMap = async () => {
    const nextOpen = !showPriorityMap;
    setShowPriorityMap(nextOpen);
    if (!nextOpen || mapLocation || !courierId) return;

    setMapLoading(true);
    try {
      const liveLocation = await getSupabaseCourierLocation(courierId);
      if (liveLocation) {
        setMapLocation({
      ...
      console.error('Error loading courier position for priority map:', error);
    } finally {
      setMapLoading(false);
    }
  };
  ...
  const geolocatedOrders = activeRouteOrders.filter(
    (order) =>
      order.locationVerified &&
      orderPoint(order) &&
      !['delivered', 'cancelled', 'failed'].includes(order.status),
  );
  const priorityOrders = prioritizeDeliveryOrders(geolocatedOrders, mapLocation);
  const recommendedOrder = priorityOrders[0] || null;
  const customerStops = priorityOrders.flatMap((order, index) => {
    const point = orderPoint(order);
    if (!point) return [];
    return [{
      id: order.id,
      name: order.customerName || 'Cliente',
      tracking: order.tracking || order.id,
      storeName: order.storeName || 'Tienda',
      address: order.formattedAddress || order.street || order.sectorName || 'Sin dirección',
      lat: point.latitude,
      lng: point.longitude,
      distanceKm: mapLocation ? distanceKm(mapLocation, point) : undefined,
      recommended: index === 0,
    }];
  });
    ...
    <div className="space-y-5 max-w-md mx-auto pb-10">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => void handleTogglePriorityMap()}
            className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[10px] font-extrabold ${
              showPriorityMap
                ? 'border-[#d3121a] bg-red-50 text-[#d3121a]'
                : 'border-[#E7E7EC] bg-white text-slate-600'
            }`}
          >
            <MapIcon size={14} />
            {showPriorityMap ? 'Cerrar mapa' : 'Ver mapa'}
          </button>
      ...
      {showPriorityMap && (
        <section className="space-y-3 rounded-2xl border border-[#E7E7EC] bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="flex items-center gap-2 text-sm font-extrabold text-slate-900">
                <LocateFixed size={16} className="text-[#d3121a]" />
                Prioridad por cercanía
              </h3>
              <p className="mt-1 text-[11px] text-slate-500">
                Sugerencia basada en tu última ubicación en vivo. Tú decides qué pedido trabajar.
              </p>
            </div>
            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600">
              {customerStops.length} con GPS
            </span>
          </div>
          ...
          {recommendedOrder && (
            <button
              type="button"
              onClick={() => void handleSelectStop(recommendedOrder)}
              ...
              <div className="mt-1 flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-extrabold text-slate-800">
                    {recommendedOrder.customerName || 'Cliente'}
                  </div>
                  <div className="text-[10px] font-semibold text-slate-500">
                    {recommendedOrder.sectorName || recommendedOrder.municipalityName}
                  </div>
                </div>
                {mapLocation && orderPoint(recommendedOrder) && (
                  <span className="text-xs font-extrabold text-emerald-700">
                    {distanceKm(mapLocation, orderPoint(recommendedOrder)!).toFixed(1)} km
                  </span>
                )}
              </div>
            </button>
          ...
          {mapLoading ? (
            <div className="flex h-[340px] items-center justify-center rounded-xl bg-slate-100 text-xs font-bold text-slate-500">
              Obteniendo ubicación del motorista...
            </div>
          ) : customerStops.length > 0 ? (
            <div className="h-[380px] overflow-hidden rounded-xl">
              <MapComponent
                activeCouriers={mapLocation ? [{
                  name: profile?.name || 'Mi ubicación',
                  status: 'En vivo',
                  lat: mapLocation.latitude,
                  lng: mapLocation.longitude,
                  pendingCount: customerStops.length,
                }] : []}
                customerStops={customerStops}
                onSelectStop={(id) => {
                  const selected = activeRouteOrders.find((order) => order.id === id);
                  if (selected) void handleSelectStop(selected);
                }}
              />
            </div>
          ) : (
            <div className="rounded-xl bg-slate-50 p-6 text-center text-xs font-semibold text-slate-500">
              Ningún pedido de esta ruta tiene una ubicación verificada.
            </div>
          )}
        </section>
      )}
